       ...
       if ($dep_id !== false) {
       		...
       		if ($ignoreUserStatus === false) {  
				...
				if (is_numeric($dep_id)) {
		           $stmt = $db->prepare("SELECT COUNT(lh_userdep.id) AS found FROM lh_userdep INNER JOIN lh_departament ON lh_departament.id = lh_userdep.dep_id WHERE (lh_departament.pending_group_max = 0 OR lh_departament.pending_group_max > lh_departament.pending_chats_counter) AND (lh_departament.pending_max = 0 OR lh_departament.pending_max > lh_departament.pending_chats_counter) AND (last_activity > :last_activity AND hide_online = 0) AND (dep_id = :dep_id {$exclipicFilter})");
			...
			if ($rowsNumber == 0) { // Perhaps auto active is turned on for some of departments
	       	...
	       	if ($ignoreUserStatus === false) {
	           $stmt = $db->prepare('SELECT COUNT(lh_userdep.id) AS found FROM lh_userdep LEFT JOIN lh_departament ON lh_departament.id = lh_userdep.dep_id WHERE (lh_departament.pending_group_max IS NULL OR lh_departament.pending_group_max = 0 OR lh_departament.pending_group_max > lh_departament.pending_chats_counter) AND (lh_departament.pending_max IS NULL OR lh_departament.pending_max = 0 OR lh_departament.pending_max > lh_departament.pending_chats_counter) AND (lh_departament.hidden IS NULL OR lh_departament.hidden = 0) AND last_activity > :last_activity AND hide_online = 0 AND (lh_departament.disabled IS NULL OR lh_departament.disabled = 0)');
	           $stmt->bindValue(':last_activity',(time()-$isOnlineUser),PDO::PARAM_INT);
	           $stmt->execute();
	           $rowsNumber = $stmt->fetchColumn();
	       }
